...
class HealthFactory {
  ...
  Future<List<HealthDataPoint>> _computeAndroidBMI(DateTime startTime, DateTime endTime, {int? limit}) async {
    List<HealthDataPoint> heights = await _prepareQuery(startTime, endTime, HealthDataType.HEIGHT, limit: limit);

    if (heights.isEmpty) {
      return [];
    }

    List<HealthDataPoint> weights = await _prepareQuery(startTime, endTime, HealthDataType.WEIGHT, limit: limit);
  ...
  /// WARNING: HAS NO EFFECT ON ANDROID - [limit]- specify the limit query for ios. Refer to - [https://stackoverflow.com/a/68801697]
  ...
  static List<HealthDataPoint> _parse(Map<String, dynamic> message) {
    ...
    final list = dataPoints.map<HealthDataPoint>((e) {
      ...
      final uuid = e.containsKey('uuid') && e['uuid'] != null ? e['uuid'] : '';
